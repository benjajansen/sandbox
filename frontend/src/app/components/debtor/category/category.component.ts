...
@Component({
  selector: 'debtorCategory'
  ,templateUrl: './category.component.html'
})
export class DebtorCategoryComponent implements OnInit{
  @ViewChild('modal') modal: ElementRef;

  categoryForm: FormGroup;
  categories : ShortDataPackage[];

  newCategory: DebtorCategory = new DebtorCategory();
  added: boolean = false;
  ...
  */
  ...
  onDelete(id: number) {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        this.debtorService.deleteCategory(this.newCategory).subscribe(
          data => {
            this.added = false;
            this.edited = false;
            this.deleted = true;
            this.deletedError = false;
            this.newCategory = new DebtorCategory();
            this.clearForm();
            this.getCategorys();
          },
  ...
  update(id: number): boolean {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        this.debtorService.updateCategory(this.newCategory).subscribe(
          data => {
            this.added = false;
            this.edited = true;
            this.deleted = false;
  ...
  onSubmit() {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        ...
        this.debtorService.sendCategory(this.newCategory).subscribe(
          data => {
            this.added = true;
            this.edited = false;
            this.deleted = false;
