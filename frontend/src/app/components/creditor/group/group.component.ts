...
@Component({
  selector: 'creditorGroup'
  ,templateUrl: './group.component.html'
})
export class CreditorGroupComponent implements OnInit{
  @ViewChild('modal') modal: ElementRef;

  groupForm: FormGroup;
  groups : ShortDataPackage[];

  newGroup: CreditorGroup = new CreditorGroup();
  added: boolean = false;
  ...
  */
  ...
  onDelete(id: number) {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        this.creditorService.deleteGroup(this.newGroup).subscribe(
          data => {
            this.added = false;
            this.edited = false;
            this.deleted = true;
            this.deletedError = false;
            this.newGroup = new CreditorGroup();
            this.clearForm();
            this.getGroups();
          },
  ...
  update(id: number): boolean {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        this.creditorService.updateGroup(this.newGroup).subscribe(
          data => {
            this.added = false;
            this.edited = true;
            this.deleted = false;
  ...
  onSubmit() {
    ...
    this.SecurityService.getUserByName(sessionStorage.getItem("currentUserName")).subscribe(
      user => {
        ...
        this.creditorService.sendGroup(this.newGroup).subscribe(
          data => {
            this.added = true;
            this.edited = false;
            this.deleted = false;
